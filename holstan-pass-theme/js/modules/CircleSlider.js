...
$(document).ready(function() {
  ...
  var titleSneak = $(".titleSneak");
  var contentSneak = $(".contentSneak");
  var imageSneak = $(".imageSneak");
  var imageDiv = $(".gallerySlide");
  var textDiv = $(".galleryInfo");
  ...
  function rotationPosition(exceptIndex) {
    ...
    sliderItems.each(function(e) {
      ...
      if (e === exceptIndex) {
        $this
          .addClass("active")
          .css("transform", "rotate(" + extraDeg + "deg)");
        textDiv.html(
          "<h1>" +
            $this.find(".titleSneak").text() +
            "</h1>" +
            "<p class='galleryContent'>" +
            $this.find(".contentSneak").html() +
            "</p>"
        );
        imageDiv.html(
          "<img class='fitBack' src=" + $this.find(".imageSneak").text() + ">"
        );
      } else {
  ...
  sliderInner.mousemove(function(e) {
    if (clickDown) {
      ...
      sliderInner
        ...
        .css("transform", "rotate(" + (motionDeg + extraDeg) + "deg)");
    }
  });
  ...
  function swipe(from, to) {
    ...
    if (distance > sliderInnerWidth / 4) {
      ...
      currentItem.css(
        "transform",
        "rotate(" + (currentItem.index() > itemsLength / 2 ? 360 : 0) + "deg)"
      );
    }
